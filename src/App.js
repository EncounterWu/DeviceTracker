...
import { Plus, Trash2, Smartphone, Laptop, Watch, Calculator, Calendar, DollarSign, X, Sparkles, Wand2, Loader2, Camera, AlertCircle, Gamepad2, Headphones, Tv, Package, Upload, Image as ImageIcon, MessageSquare, Home, User, PieChart } from 'lucide-react';
...
export default function App() {
  ...
  const [newItem, setNewItem] = useState({ name: '', price: '', date: '', type: 'phone', image: null });
  ...
  const handleSmartAdd = async () => {
    ...
    try {
      ...
      console.error(e);
      setErrorMsg(e.message === "请先配置 DeepSeek API Key" ? "请在代码中填入 API Key" : "AI 识别失败，请重试。");
    } finally {
  ...
  const getTypeColor = (type) => {
    switch (type) {
      case 'laptop': return '#3B82F6'; // blue-500
      case 'watch': return '#A855F7'; // purple-500
      case 'console': return '#6366F1'; // indigo-500
      case 'camera': return '#F43F5E'; // rose-500
      case 'audio': return '#EC4899'; // pink-500
      case 'home': return '#F59E0B'; // amber-500
      case 'other': return '#94A3B8'; // slate-400
      default: return '#10B981'; // emerald-500 (phone)
    }
  };

  const getTypeLabel = (type) => {
    const map = {
      phone: '手机', laptop: '电脑/平板', watch: '穿戴', console: '游戏',
      camera: '摄影', audio: '音频', home: '家电', other: '其他'
    };
    return map[type] || '其他';
  };
  ...
  const getIcon = (type) => {
    ...
    switch (type) {
      ...
      case 'console': return <Gamepad2 {...props} />;
      case 'camera': return <Camera {...props} />;
      case 'audio': return <Headphones {...props} />;
      case 'home': return <Tv {...props} />;
      case 'other': return <Package {...props} />;
      default: return <Smartphone {...props} />;
    }
  };

  const totalSpent = items.reduce((acc, item) => acc + parseFloat(item.price || 0), 0);
  ...
  const SimplePieChart = ({ items }) => {
    if (items.length === 0) return <div className="text-center text-slate-400 py-10">暂无数据</div>;
    ...
    const slices = Object.entries(categoryData)
      ...
      .map(([type, value]) => {
        ...
        const slice = { path: pathData, color: getTypeColor(type), type, value, percentage };
        startAngle = endAngle;
        return slice;
      });
      ...
      if (slices.length === 1) {
        return (
             <div className="flex items-center justify-center py-6">
                <div className="w-48 h-48 rounded-full" style={{ backgroundColor: slices[0].color }}></div>
             </div>
        )
      }
    ...
    return (
      <div className="flex flex-col items-center py-4">
        <div className="relative w-48 h-48">
          <svg viewBox="-1 -1 102 102" className="transform -rotate-90 w-full h-full">
            {slices.map((slice, i) => (
          ...
          {slices.map((slice, i) => (
            <div key={i} className="flex items-center justify-between text-sm">
              <div className="flex items-center gap-2">
                ...
                <span className="text-slate-600">{getTypeLabel(slice.type)}</span>
              </div>
              <span className="font-bold text-slate-700">{Math.round(slice.percentage * 100)}%</span>
            </div>
          ))}
        </div>
      </div>
  ...
  const HomePage = () => (
    <div className="p-4 max-w-md mx-auto pb-24 animate-in fade-in duration-300">
        ...
        <div className="bg-indigo-600 rounded-2xl p-6 text-white shadow-lg mb-6">
          ...
          <div className="text-3xl font-bold mb-4">¥ {totalSpent.toLocaleString()}</div>
        ...
        {/* --- 设备列表 --- */}
        <h2 className="text-slate-500 text-sm font-semibold mb-3 px-1">设备清单</h2>
        {items.length === 0 ? (
          ...
          <div className="space-y-3">
            {items.map(item => {
              ...
              return (
                <div key={item.id} className="bg-white p-4 rounded-xl shadow-sm border border-slate-100 relative group transition-all active:scale-95">
                  <div className="flex items-start justify-between mb-2">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="overflow-hidden">
                        ...
                        <p className="text-xs text-slate-400">购买于 {item.date}</p>
                      </div>
                    </div>
                    <button 
                      onClick={(e) => {
                  ...
                  <div className="grid grid-cols-2 gap-4 mt-3 bg-slate-50 p-3 rounded-lg">
                    <div className="text-center border-r border-slate-200">
                      ...
                      <div className="text-indigo-600 font-bold">
                        {stats.days} <span className="text-xs font-normal text-slate-400">天</span>
                      </div>
                    </div>
                    <div className="text-center">
                      <div className="text-xs text-slate-500 mb-1">日均成本</div>
                      <div className="text-emerald-600 font-bold">
                        ¥{stats.dailyCost} <span className="text-xs font-normal text-slate-400">/天</span>
                      </div>
                    </div>
                  </div>
                </div>
    ...
    <div className="p-4 max-w-md mx-auto pb-24 animate-in fade-in duration-300">
       <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 mb-6">
          ...
          <div className="grid grid-cols-2 gap-4">
             ...
             <div className="bg-slate-50 p-4 rounded-xl">
               <div className="text-xs text-slate-500 mb-1">总日均消耗 (元/天)</div>
               <div className="text-2xl font-bold text-rose-500">{totalDailyCost.toFixed(2)}</div>
             </div>
          </div>
       </div>
    ...
    <div className="min-h-screen bg-slate-100 font-sans text-slate-800 relative overflow-hidden flex flex-col">
      ...
      <div className="fixed bottom-0 w-full bg-white border-t border-slate-200 flex justify-around items-center pb-safe pt-2 z-30 h-[80px]">
        <button 
          ...
          className={`flex flex-col items-center gap-1 p-2 w-20 ${activeTab === 'home' ? 'text-indigo-600' : 'text-slate-400 hover:text-slate-600'}`}
        >
        ...
        <button 
          ...
          className={`flex flex-col items-center gap-1 p-2 w-20 ${activeTab === 'mine' ? 'text-indigo-600' : 'text-slate-400 hover:text-slate-600'}`}
        >
          ...
          <div className="bg-white rounded-2xl w-full max-w-sm overflow-hidden shadow-2xl animate-in fade-in zoom-in duration-200 flex flex-col max-h-[90vh]">
            ...
            <div className="flex border-b border-slate-100 shrink-0">
              <button 
                ...
                className={`flex-1 py-3 text-sm font-medium ${modalMode === 'manual' ? 'text-indigo-600 border-b-2 border-indigo-600' : 'text-slate-500'}`}
              >
                普通录入
              </button>
              <button 
                onClick={() => setModalMode('ai')}
                className={`flex-1 py-3 text-sm font-medium flex items-center justify-center gap-1 ${modalMode === 'ai' ? 'text-purple-600 border-b-2 border-purple-600' : 'text-slate-500'}`}
              >
              ...
              {modalMode === 'ai' ? (
                <div className="space-y-4">
                   <div className="bg-blue-50 p-4 rounded-xl text-xs text-blue-700 mb-2 border border-blue-100">
                    <p className="font-bold mb-1 flex items-center gap-1"><MessageSquare size={12}/> DeepSeek 模式：</p>
                    <p className="opacity-80">请输入一段话，AI 会自动提取设备名、价格和日期。</p>
                    <p className="opacity-60 mt-1 text-[10px]">* 暂不支持图片识别，请直接描述，如“昨天京东3000买了个耳机”。</p>
                  </div>
                ...
                <form onSubmit={handleAddItem} className="space-y-4">
                  <div>
                    <label className="block text-sm text-slate-500 mb-1">设备图片 (可选)</label>
                    <div 
                      onClick={() => manualFileInputRef.current?.click()}
                      className={`border border-dashed rounded-xl p-3 flex flex-col items-center justify-center cursor-pointer transition-colors ${newItem.image ? 'border-indigo-300 bg-indigo-50' : 'border-slate-300 hover:border-indigo-400 hover:bg-slate-50'}`}
                    >
                      <input 
                        type="file" 
                      ...
                      {newItem.image ? (
                        <div className="relative w-full">
                          <img src={newItem.image} alt="Preview" className="h-32 rounded-lg mx-auto object-contain shadow-sm" />
                          <button 
                            type="button"
                            onClick={(e) => {
                              e.stopPropagation();
                  ...
                  <div>
                    <label className="block text-sm text-slate-500 mb-1">设备名称</label>
                    <input 
                      type="text" required placeholder="例如：MacBook Air M2"
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="block text-sm text-slate-500 mb-1">价格 (元)</label>
                      <div className="relative">
                        <DollarSign size={16} className="absolute left-3 top-3.5 text-slate-400" />
                        <input 
                          type="number" required placeholder="8999"
                    ...
                    <div>
                      <label className="block text-sm text-slate-500 mb-1">设备类型</label>
                      <select 
                  ...
                  <div>
                    <label className="block text-sm text-slate-500 mb-1">购买日期</label>
                    <div className="relative">
                  ...
                  <button type="submit" className="w-full bg-indigo-600 text-white py-3 rounded-xl font-bold hover:bg-indigo-700 active:bg-indigo-800 transition-colors mt-2">确认添加</button>
                </form>
              )}
            </div>
          </div>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-xs shadow-2xl p-6 animate-in fade-in zoom-in duration-200">
            <div className="flex flex-col items-center text-center mb-6">
              ...
              <p className="text-sm text-slate-500 mt-1">该设备的记录将被永久移除，无法恢复。</p>
            </div>
            <div className="flex gap-3">
              <button onClick={() => setItemToDelete(null)} className="flex-1 py-2.5 bg-slate-100 text-slate-600 rounded-xl font-medium hover:bg-slate-200 transition-colors">取消</button>
              <button onClick={confirmDelete} className="flex-1 py-2.5 bg-red-500 text-white rounded-xl font-medium hover:bg-red-600 shadow-lg shadow-red-200 transition-colors">删除</button>
            </div>
          </div>
        </div>
      )}
